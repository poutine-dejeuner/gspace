...
for l in LAYERS:
    ...
    # Score: s_i[t] = j_vecs[t] @ acts[i]
    # For memory efficiency: batch by 200 samples
    batch_size = 200
    ranks = []
    top1_count = 0
    n_valid = 0
    ...
    for start in range(0, n, batch_size):
        ...
        scores = batch_acts @ j_vecs.T  # [batch, vocab]

        for i in range(len(batch_acts)):
            ans = int(batch_labels[i])
...
try:
    ...
    # Normalize all J-lens vectors
    j_norms = np.linalg.norm(j_vecs_peak, axis=1, keepdims=True)
    j_normed = j_vecs_peak / np.where(j_norms > 1e-8, j_norms, 1.0)
    ...
    token_usage = np.zeros(vocab_size)
    ...
    top_tokens_idx = np.argsort(token_usage)[-30:][::-1]
    top_tokens = [(int(idx), token_usage[idx] / n_decomp) for idx in top_tokens_idx]
    ...
    for tok_id, freq in top_tokens[:20]:
    ...
    k_subspace = min(100, len(top_tokens_idx))
    top_j_indices = sorted(top_tokens_idx[:k_subspace])
    V_subspace = j_normed[top_j_indices].astype(np.float64)
    ...
    sparse_results = {
        ...
        "top_tokens": [(int(idx), float(freq)) for idx, freq in top_tokens[:30]],
...
plt.tight_layout()
plot_path = OUTPUT_DIR / "jspace_analysis.png"
...
plot_path2 = OUTPUT_DIR / "gspace_vs_jspace_peaks.png"
